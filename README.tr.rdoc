...
Sinatra çok az çaba harcayarak web uygulamalarını Ruby ile yapabileceğiniz bir DSL dir:
...
Dosyayı utf-8 olarak kaydetmeyi unutmayın. Gem kurulumu yapın ve programı çalıştırın:

  gem install sinatra
  ruby -rubygems myapp.rb

Tarayıcıda şu adrese gidin: http://localhost:4567

Ayrıca <tt>gem install thin</tt> komutu ile thin web server kurulumu tavsiye edilir, Sinatra 
eğer varsa bunu kullanacaktır.

== Yönlendirmeler

Sinatra'da , yönlendirmeler URL eşleniği bir HTTP metod bloğudur.
Her yönlendirme bir blok ile ifade edilir:
...
Yönlendirmeler tanımlandığı sırayla eşlenirler. Çağrıya uyan ilk yönlendirme çalışacaktır.

Yönlendirmeler 
<tt>params</tt> hash ile erişilebilen parametreler içerebilir:

  get '/hello/:name' do
    #  "GET /hello/foo" ve "GET /hello/bar" adreslerine eşlenir
    # params[:name] de 'foo' veya 'bar' olacaktır
    "Merhaba #{params[:name]}!"
  end

Ayrıca isimlendirilmiş parametrelere blok parametreleri üzerinden de erişebilirsiniz:

  get '/hello/:name' do |n|
    "Merhaba #{n}!"
  end

Yönlendirme paternleri ayrıca <tt>params[:splat]</tt> array ile erişilebilen 
splat (ya da wildcard) parametreler de içerebilir:

  get '/say/*/to/*' do
    # /say/hello/to/world adresine eşleşir
    params[:splat] # => ["hello", "world"]
  end

  get '/download/*.*' do
    # /download/path/to/file.xml adresiyle eşleşir
    params[:splat] # => ["path/to/file", "xml"]
  end

Ya da aynı şeyi blok parametreleriyle yaparsak:

  get '/download/*.*' do |path, ext|
    [path, ext] # => ["path/to/file", "xml"]
  end

Regular Expression ile yönlendirme:

  get %r{/hello/([\w]+)} do
    "Hello, #{params[:captures].first}!"
  end

Aynısı blok parametresi ile yapılırsa:

  get %r{/hello/([\w]+)} do |c|
    "Hello, #{c}!"
  end

Yönlendirme paternleri opsiyonel parametreler de alabilir:

  get '/posts.?:format?' do
    # "GET /posts" ve herhengi bir ekli hal ile eşleşir "GET /posts.json", "GET /posts.xml" gibi.
  end

Sırası gelmişken, eğer path traversal atak korumasını aktif etmediyseniz (aşağıda), 
çağrılan path sizin yönlendirmelerinizi karşılayacak şekilde değiştirilmiş olabilir.

=== Koşullar

Yönlendirmeler birçok koşullu eşleşme yapabilir, örneğin "user agent":

  get '/foo', :agent => /Songbird (\d\.\d)[\d\/]*?/ do
    "Songbird version #{params[:agent][0] kullanıyorsunuz}"
  end

  get '/foo' do
    # songbird olmayan tarayıcılar buraya gelir
  end

Diğer olası koşullar +host_name+ ve +provides+:

  get '/', :host_name => /^admin\./ do
    "Admin bölgesi, Erişim yasaktır!"
  end
...
Kendi koşullarınızı kolayca oluşturabilirsiniz:
...
Birden fazla değer içeren koşullar için splat kullanın:
  ...
  get "/my/account/", :auth => [:user, :admin] do
    "user ve admin buraya erişir"
  end

  get "/only/admin/", :auth => :admin do
    "Sadece admin buraya erişir!"
  end

=== Döndürülen Değerler

Bir yönlendirme bloğunun dönen değeri en azından HTTP client'a gönderilen bir 
cevap sayfası gövdesidir. Yukardaki örneklerdeki gibi çoğunlukla bir string 
değer döner, fakat diğer değerler de olabilir.

Herhangi bir nesne dönebilirsiniz. Bu bir Rack response, Rack body nesnesi ya da 
HTTP status kod olabilir:

* Üç elemanlı bir array: <tt>[status (Fixnum), headers (Hash), response
  body (#each metoduna cevap verir)]</tt>
* iki elemanlı bir array: <tt>[status (Fixnum), response body (#each metoduna 
cevap verir)]</tt>
* <tt>#each</tt> metoduna cevap veren bir nesne ve verilen bloğa sadece string 
gönderir
* Status kodu belirten bir Fixnum

Bu sayede çok kolayca bir sıralama örneği verebiliriz:
...
Yukarda göründüğü gibi Sinatra regular expressionlar ve String paternleri 
yönlendirme için kullanabilecek şekilde paketlenmiş. Ama sadece bu kadar değil. 
Kendi eşleştirmelerinizi de tanımlayabilirsiniz:
...
Not, bu yukardaki örnekte gereksiz mühendislik yapılmıştır. Bunu şu şekillerde 
de yapabiliriz:
...
Static dosyalar <tt>./public</tt> klasöründen yayınlanır. Başka bir yeri 
ayarlamak için <tt>:public_folder</tt> opsiyonu kullanılabilir:

  set :public_folder, File.dirname(__FILE__) + '/static'

Not bu public klasör adı URL içinde kullanılmaz. Örneğin <tt>./public/css/style.css</tt> 
dosyasını ifade etmek için <tt>http://example.com/css/style.css</tt> kullanılır.

<tt>Cache-Control</tt> header bilgisi eklemek için <tt>:static_cache_control</tt> 
ayarını kullanın.

== Görseller  / Şablonlar

Her şablon dili kendi yayımlama metodu kullanılarak gösterilir. Bu metodlar 
basit bir şekilde string geri dönerler:

  get '/' do
    erb :index
  end

Bu satırlar <tt>views/index.erb</tt> dosyasını geri döner.

Şablon adı yerine şablonun içeriğini direk olarak girebilirsiniz:
...
Şablonlar opsiyonlar hash'i olarak ikinci bir argüman alır:

  get '/' do
    erb :index, :layout => :post
  end

Bu satırlar ile <tt>views/index.erb</tt> dosyasını <tt>views/post.erb</tt> 
yerleşimi içinde sunarsınız (default olarak eğer varsa 
<tt>views/layout.erb</tt> dosyası kullanılır).

Sinatra tarafından anlaşılamayan herhangibir opsiyon şablon işleyiciye 
gönderilecektir:
...
Ayrıca her şablon dili için genel opsiyon ayarı yapabilirsiniz:
...
İşleyici metoda gönderilen opsiyonlar +set+ ile girilenlerin üstüne yazacaktır.
